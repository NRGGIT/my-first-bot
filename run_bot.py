...
def message_interpreter(text, word_topic_dic,
                        topic_group_dic):  # нужна для того, чтобы распознавать слова по корням (пример: скинь котейку. видит, находит "кот" в word_topic_dic и его значение находит в GroupsDic, где получает id нужной группы)
    text = text.lower()
...
def find_best_fresh_post(owner_id,
                         user):  # поиск лучшего поста в заданной группе за последние сутки. сохраняет id поста в IDHistory для данного пользователя, чтобы потом не скидывать тот же пост при таком же запросе.
    posts = json.loads((requests.get(
        'https://api.vk.com/method/wall.get?owner_id={owner_id}&count={count}&filter={filter}&access_token={token}&v={version}'.format(
            owner_id=owner_id,
            count=40,
            filter='all',
            token=ACCAUNT_TOKEN,
...
def write_msg(user_id, text):
    requests.get(
        'https://api.vk.com/method/messages.send?user_id={id}&message={text}&access_token={token}&v={version}'.format(
            id=user_id,
            text=text,
            token=GROUP_TOKEN,
            version='5.80'))
...
def send_post(user_id, text, attach):
    requests.get(
        'https://api.vk.com/method/messages.send?user_id={id}&message={text}&attachment={attach}&access_token={token}&v={version}'.format(
            id=user_id,
            text=text,
            attach=attach,
            token=GROUP_TOKEN,
...
def lp_update():
    return json.loads((requests.get(
        'https://api.vk.com/method/messages.getLongPollServer?need_pts={need_pts}&lp_version={lp_version}&access_token={token}&v={version}'.format(
            need_pts=1,
            lp_version=3,
            token=GROUP_TOKEN,
            version='5.80'))).text)['response']
...
while True:
    ...
    response = json.loads((requests.get(
        'https://{server}?act=a_check&key={key}&ts={ts}&wait=20ms&mode=2&version=3'.format(server=lp_server['server'],
                                                                                           key=lp_server['key'],
